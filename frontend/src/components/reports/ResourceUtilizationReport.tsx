...
export function ResourceUtilizationReport({ data }: ResourceUtilizationReportProps) {
  const utilizationStatus = useMemo(() => {
    if (data.avgUtilization >= 80) return { status: "Optimal", color: "bg-green-100 text-green-700" };
    if (data.avgUtilization >= 60) return { status: "Good", color: "bg-blue-100 text-blue-700" };
    if (data.avgUtilization >= 40) return { status: "Fair", color: "bg-yellow-100 text-yellow-700" };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-purple-50 to-blue-50 p-4 rounded-lg border border-purple-200">
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-sm text-gray-600">Resource Utilization Analysis</p>
          </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        <div className="text-center mb-6">
          <p className="text-gray-600 mb-2">Average Utilization</p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4">
          <p className="text-xs text-gray-500 uppercase tracking-wider">Fully Utilized</p>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4">
          <p className="text-xs text-gray-500 uppercase tracking-wider">Underutilized</p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {resourcesByType.map((rt, idx) => (
              <div key={idx} className="bg-gray-50 rounded-lg p-3 text-center">
                <p className="text-sm text-gray-600">{rt.type}</p>
                <p className="text-2xl font-bold text-gray-900">{rt.count}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4">
        <h4 className="font-semibold text-gray-900 mb-4">Individual Resource Utilization</h4>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200">
                <th className="text-left py-3 px-2 text-gray-600 font-semibold">Code</th>
                <th className="text-left py-3 px-2 text-gray-600 font-semibold">Name</th>
                <th className="text-left py-3 px-2 text-gray-600 font-semibold">Type</th>
                <th className="text-right py-3 px-2 text-gray-600 font-semibold">Planned Hours</th>
                <th className="text-right py-3 px-2 text-gray-600 font-semibold">Actual Hours</th>
                <th className="text-center py-3 px-2 text-gray-600 font-semibold">Utilization</th>
              </tr>
            </thead>
            <tbody>
              {data.resources.map((resource, idx) => (
                <tr key={idx} className="border-b border-gray-100 hover:bg-gray-50">
                  <td className="py-3 px-2 font-mono text-gray-900">{resource.code}</td>
                  <td className="py-3 px-2 text-gray-700">{resource.name}</td>
                  <td className="py-3 px-2 text-gray-600">{resource.type}</td>
                  <td className="py-3 px-2 text-right text-gray-700">{resource.plannedHours.toFixed(1)}</td>
                  <td className="py-3 px-2 text-right text-gray-700">{resource.actualHours.toFixed(1)}</td>
