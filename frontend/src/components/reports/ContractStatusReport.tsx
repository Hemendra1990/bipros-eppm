    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-50 to-cyan-50 p-4 rounded-lg border border-blue-200">
        <div>
          ...
          <p className="text-sm text-slate-400">Contract & Variation Order Status</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-slate-900/50 border border-slate-800 rounded-lg p-4">
          <p className="text-xs text-slate-500 uppercase tracking-wider">Total Contracts</p>
          <p className="text-2xl font-bold text-white">{data.totalContracts}</p>
          <p className="text-xs text-emerald-400 mt-1">{data.activeContracts} active</p>
        </div>

        <div className="bg-slate-900/50 border border-slate-800 rounded-lg p-4">
          <p className="text-xs text-slate-500 uppercase tracking-wider">Total Value</p>
          <p className="text-2xl font-bold text-blue-400">${data.totalContractValue.toFixed(0)}</p>
        </div>

        <div className="bg-slate-900/50 border border-slate-800 rounded-lg p-4">
          <p className="text-xs text-slate-500 uppercase tracking-wider">VO Value</p>
        ...
        <div className="bg-slate-900/50 border border-slate-800 rounded-lg p-4">
          ...
          <p className="text-2xl font-bold text-white">{data.achievedMilestones}/{data.achievedMilestones + data.pendingMilestones}</p>
          <p className="text-xs text-slate-400 mt-1">{milestoneCompletion.toFixed(0)}% complete</p>
        </div>
      </div>
      ...
      <div className="bg-slate-900/50 border border-slate-800 rounded-lg p-4">
        ...
        {data.contracts.length > 0 ? (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-800">
                  <th className="text-left py-3 px-2 text-slate-400 font-semibold">Contract #</th>
                  <th className="text-left py-3 px-2 text-slate-400 font-semibold">Contractor</th>
                  <th className="text-right py-3 px-2 text-slate-400 font-semibold">Value</th>
                  <th className="text-left py-3 px-2 text-slate-400 font-semibold">Status</th>
                  <th className="text-right py-3 px-2 text-slate-400 font-semibold">Pending Milestones</th>
                </tr>
              </thead>
              <tbody>
                {data.contracts.map((contract, idx) => (
                  <tr key={idx} className="border-b border-slate-800/50 hover:bg-slate-900/80">
                    <td className="py-3 px-2 font-mono text-white">{contract.contractNumber}</td>
                    <td className="py-3 px-2 text-slate-300">{contract.contractor}</td>
          ...
          <p className="text-slate-500 text-center py-4">No contracts found</p>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-slate-900/50 border border-slate-800 rounded-lg p-4">
          <h4 className="font-semibold text-white mb-3">Contract Summary</h4>
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between">
              <span className="text-slate-400">Inactive/Completed</span>
            ...
            <div className="flex justify-between border-t pt-2 mt-2">
              ...
              <span className="font-semibold text-blue-400">${data.totalContractValue.toFixed(0)}</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-900/50 border border-slate-800 rounded-lg p-4">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              <span className="text-slate-400">Total VO Value</span>
              <span className="font-semibold text-orange-600">${data.totalVoValue.toFixed(0)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-400">% of Contract Value</span>
