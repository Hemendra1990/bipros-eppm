...
export default function RiskRegisterReportPage() {
  ...
  const { data: typesData } = useQuery({
    queryKey: ["risk-category-types"],
    queryFn: () => riskCategoryApi.listTypes(),
  });
  const riskTypes = typesData?.data ?? [];

  const { data: categoriesData } = useQuery({
    queryKey: ["risk-categories", selectedTypeId, recommendedIndustry],
    queryFn: () =>
      riskCategoryApi.listCategories({
        typeId: selectedTypeId || undefined,
        industry: recommendedIndustry,
      }),
    enabled: showForm && !!selectedTypeId,
  });
  const filteredCategories = categoriesData?.data ?? [];
  const categoryOptions: SelectOption[] = filteredCategories.map((c) => ({
    value: c.id,
    label: `${c.name}  (${c.industry} · ${c.code})`,
  }));
  ...
  const handleFormChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    if (name === "impact") {
      ...
      setFormData((prev) => ({ ...prev, impactCost: v, impactSchedule: v }));
      return;
    }
    setFormData((prev) => ({
      ...prev,
      [name]: name === "probability" || name === "impactCost" || name === "impactSchedule"
        ? parseInt(value, 10)
        : value,
    }));
  };
  ...
  return (
    <div>
      <PageHeader
        title="Cross-Project Risk Register"
        description="Portfolio-level risk management across all projects"
        actions={
      ...
      {showForm && selectedProjectId && (
        <div className="mb-8 rounded-xl border border-border bg-surface/50 p-6 shadow-lg">
          ...
          <form onSubmit={handleFormSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-text-secondary">Category Type</label>
                <select
              ...
              <div>
                ...
                <div className="mt-1">
                  <SearchableSelect
                    ...
                    placeholder={selectedTypeId ? "Search categories…" : "Select a type first"}
                  />
                </div>
              </div>
