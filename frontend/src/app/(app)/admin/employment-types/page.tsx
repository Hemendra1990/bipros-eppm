...
import {
  employmentTypeMasterApi,
  type EmploymentTypeMaster,
  type EmploymentTypeMasterRequest,
} from "@/lib/api/employmentTypeMasterApi";
...
const formFromRow = (r: EmploymentTypeMaster): RowForm => ({
  code: r.code,
  name: r.name,
...
export default function EmploymentTypesPage() {
  ...
  const openEdit = (row: EmploymentTypeMaster) => {
    setEditingId(row.id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const payload: EmploymentTypeMasterRequest = {
        code: form.code.trim(),
        name: form.name.trim(),
      ...
      if (editingId) {
        await employmentTypeMasterApi.update(editingId, payload);
      } else {
        await employmentTypeMasterApi.create(payload);
      }
      closeForm();
      queryClient.invalidateQueries({ queryKey: ["employment-types"] });
    } catch (err: unknown) {
      setError(getErrorMessage(err, "Failed to save employment type"));
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await employmentTypeMasterApi.delete(id);
  ...
  return (
    <div>
      <TabTip
        ...
        description="Master list of employment types — Permanent, Contract, Daily Wage, etc. Used as a dropdown on the Manpower resource form."
      />
      ...
      <div className="mb-8 flex items-start justify-between gap-6">
        <div>
          ...
          <p className="mt-2 max-w-[560px] text-sm text-slate leading-relaxed">
            Defines the dropdown options on the Manpower resource form&apos;s Employment Type field.
          </p>
        </div>
        <button
      ...
      {isError && (
        <div className="mb-4 rounded-xl border border-burgundy/30 bg-burgundy/10 p-4 text-sm">
          <div className="font-medium text-burgundy">Failed to load employment types</div>
      ...
      {!isLoading && filtered.length === 0 && (
        <div className="rounded-xl border border-dashed border-hairline bg-paper py-12 text-center">
          <p className="text-sm text-slate">
            {rows.length === 0
              ? "No employment types yet. Add your first one to get started."
