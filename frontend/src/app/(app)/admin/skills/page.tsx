...
import {
  skillMasterApi,
  type SkillMaster,
  type SkillMasterRequest,
} from "@/lib/api/skillMasterApi";
...
const formFromRow = (r: SkillMaster): RowForm => ({
  code: r.code,
  name: r.name,
...
export default function SkillsPage() {
  ...
  const openEdit = (row: SkillMaster) => {
    setEditingId(row.id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const payload: SkillMasterRequest = {
        code: form.code.trim(),
        name: form.name.trim(),
      ...
      if (editingId) await skillMasterApi.update(editingId, payload);
      else await skillMasterApi.create(payload);
      closeForm();
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await skillMasterApi.delete(id);
  ...
  return (
    <div>
      <TabTip
        title="Skills"
        description="Master list of skills (Masonry, Welding, Surveying, etc.). Used for both Primary and Secondary Skills multi-select pickers on the Manpower resource form."
      />
      ...
      <div className="mb-8 flex items-start justify-between gap-6">
        <div>
          ...
          <p className="mt-2 max-w-[560px] text-sm text-slate leading-relaxed">
            One source of skill options used by both the Primary Skills and Secondary Skills
            pickers on the Manpower resource form.
          </p>
        </div>
        <button
      ...
      {isError && (
        <div className="mb-4 rounded-xl border border-burgundy/30 bg-burgundy/10 p-4 text-sm">
          <div className="font-medium text-burgundy">Failed to load skills</div>
